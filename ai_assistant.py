...
class AIPredictor(QThread):
    ...
    def __init__(self):
        ...
        # YOLO類別映射到我們的車輛類別 (更精確的映射)
        self.yolo_to_vehicle_mapping = {
            0: 0,   # YOLO person -> 忽略
            1: 1,   # YOLO bicycle -> 忽略  
            2: 1,   # YOLO car -> 汽車
            3: 0,   # YOLO motorcycle -> 機車  
            4: 1,   # YOLO airplane -> 忽略
            5: 3,   # YOLO bus -> 公車
            6: 1,   # YOLO train -> 忽略
            7: 2,   # YOLO truck -> 卡車
        }
        
        # 車輛類別名稱 (0=機車, 1=汽車, 2=卡車, 3=公車)
    ...
    def parse_predictions(self, result, image_path: str) -> List[Dict]:
        ...
        for i, (box, conf, cls) in enumerate(zip(boxes, confidences, classes)):
            ...
            if cls in [2, 3, 5, 7]:  # car, motorcycle, bus, truck
                ...
                class_name = self.vehicle_classes[vehicle_class]
