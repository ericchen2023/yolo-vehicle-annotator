        ...
        try:
            ...
            if hasattr(self.image_cache, 'cache') and self.image_cache.cache:
                for path in list(self.image_cache.cache.keys())[:10]:  # 只顯示前10項
                    items.append({
                        'path': path,
                        'size': cache_stats['current_size'] / max(cache_stats['item_count'], 1) / (1024 * 1024)  # 估算每項大小
                    })
            
            return {
                'size': cache_stats['item_count'],  # main.py期望的鍵
                'memory_usage': cache_stats['current_size'] / (1024 * 1024),  # main.py期望的鍵
                'max_size_mb': cache_stats['max_size'] / (1024 * 1024),
                'hit_rate': cache_stats['hit_rate'],
                'usage_percent': (cache_stats['current_size'] / cache_stats['max_size']) * 100,
